...
app.post('/api/tours', isAdmin, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            ...
            [title, price, duration, imageUrl || 'https://images.pexels.com/photos/258154/pexels-photo-258154.jpeg', description, itinerary]
        );
...
app.put('/api/tours/:id', isAdmin, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            ...
            [title, price, duration, imageUrl, description, itinerary, req.params.id]
        );
        res.json(result.rows[0]);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/activities', isAdmin, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            ...
            [title, price, duration, imageUrl || 'https://images.pexels.com/photos/258154/pexels-photo-258154.jpeg', description, included, highlights, whatToBring, additionalInfo, terms]
        );
        res.json(result.rows[0]);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/api/activities/:id', isAdmin, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            ...
            [title, price, duration, imageUrl, description, included, highlights, whatToBring, additionalInfo, terms, req.params.id]
        );
        res.json(result.rows[0]);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/villas', isAdmin, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            ...
            [name, price, location, imageUrl || 'https://images.pexels.com/photos/258154/pexels-photo-258154.jpeg', facilities, description]
        );
        res.json(result.rows[0]);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/api/villas/:id', isAdmin, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            ...
            [name, price, location, imageUrl, facilities, description, req.params.id]
        );
        res.json(result.rows[0]);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
